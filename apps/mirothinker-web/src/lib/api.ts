...
// Auth functions
export async function enterPassword(password: string, username: string): Promise<{ token: string; username: string }> {
  const response = await apiClient.post<{ access_token: string; username: string }>('/api/auth/enter', {
    password,
    username,
  });
  ...
  return { token: access_token, username: name };
}
